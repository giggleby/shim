...
# Script to increase file system size on a Chromium OS disk image.
...
DEFINE_integer increase_mb "1024" \
  "Size in MB to be increased." "m"
...
increase_filesystem_size() {
  ...
  local new_size="$((size_mb + increase_mb))" ||
    die "Failed to calculate new file system size (${size_mb} + ${increase_mb})"

  sudo resize2fs "${partition}" "${new_size}M" ||
...
main() {
  ...
  increase_filesystem_size "${FLAGS_image}" "${FLAGS_partition_index}" \
    "${FLAGS_increase_mb}"
}
