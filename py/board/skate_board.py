...
class SkateBoard(ChromeOSBoard):
  """Board interface for Skate."""
  def __init__(self):
    super(SkateBoard, self).__init__()
  ...
  def SetChargeState(self, state):
    """Sets charge state for skate board.
  ...
  def GetChargerCurrent(self):
    """Charger current is not available on skate board."""
    raise NotImplementedError

  def GetPowerInfo(self):
    """Gets ectool powerinfo on skate board."""
    try:
  ...
  def GetBatteryRegisters(self):
    """Gets battery registers on skate board.
