...
# Cleanup script is created during the build phase of the toolkit, and is
# removed after the build phase on the builder.
CLEANUP_SCRIPT_PATH = './factory_installer_cleanup.sh'
# EXTRACTED_PATH is the path where the makeself extracted to. Maekself extracts
# content to tmp directory by default, but since the symbolic link is restricted
# under /tmp, we changed the extracted path under /usr/local/tmp, see
# b/235148382.
EXTRACTED_PATH = '/usr/local/tmp/factory_toolkit'

# Short and sweet help header for the executable generated by makeself.
HELP_HEADER = """
- To enable factory environment for a device, run the following command.  The
  factory environment will come up after reboot.

    ssh DUT_IP '/usr/local/factory-toolkit/install_factory_toolkit.run'
    ssh DUT_IP 'sync && reboot'

- To enable locally patched factory environment for a device, copy
  install_factory_toolkit.run to the device and run it with the following
  command.  The locally patched factory environment will come up after reboot.

    ssh DUT_IP 'mkdir /usr/local/tmp'
    rsync -a install_factory_toolkit.run DUT_IP:/usr/local/tmp
    ssh DUT_IP '/usr/local/tmp/install_factory_toolkit.run && sync && reboot'

- To enable factory environment before flashing the image, modify the test image
  with the following command and flash it to the device.  The factory
  environment will come up first boot.

    install_factory_toolkit.run chromiumos_test_image.bin
    cros flash DUT_IP chromiumos_test_image.bin
"""

HELP_HEADER_ADVANCED = """
- (advanced) To enable factory environment before flashing the image, modify the
  test image by mounting the stateful partition with the following command and
  flash it to the device.  The factory environment will come up first boot.
...
- (advanced) Unpack the factory toolkit, modify a file, and then repack it.
    ...
    install_factory_toolkit.run --target /usr/local/tmp/toolkit --noexec
    # Edit some files in /usr/local/tmp/toolkit
    vim /usr/local/tmp/toolkit/whatever
    # Repack
    install_factory_toolkit.run -- --repack /usr/local/tmp/toolkit \\
...
CLEANUP_SCRIPT = f"""
#!/bin/bash
# This is the cleanup script for ChromeOS install_factory_toolkit.run that runs
# when execution is interrupted or finishes successfully.
echo 'Cleanup mid-product produced by install_factory_toolkit.run...'
...
def PackFactoryToolkit(src_root, output_path, initial_version, quiet=False):
  ...
  with file_utils.TempDirectory() as tmp_dir:
    ...
    cleanup_script_path = os.path.join(tmp_dir, CLEANUP_SCRIPT_PATH)
    file_utils.WriteFile(cleanup_script_path, CLEANUP_SCRIPT)
    file_utils.SetFileExecutable(cleanup_script_path)
