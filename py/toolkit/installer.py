...
class FactoryToolkitInstaller(object):
  ...
  def Install(self):
    ...
    for src, dest in ((self._usr_local_src, self._usr_local_dest),):
      ...
      try:
        if self._sudo:
          process_utils.Spawn(['chown', '-R', 'root', src],
                              sudo=True, log=True, check_call=True)
          process_utils.Spawn(['chmod', '-R', 'go+rX', src],
                              sudo=True, log=True, check_call=True)
        print '***   %s -> %s' % (src, dest)
        process_utils.Spawn(['rsync', '-a', '--force'] + SERVER_FILE_MASK +
        ...
        if self._sudo:
          ...
          process_utils.Spawn(['chown', '-R', myuser, src],
                              sudo=True, log=True, check_call=True)
