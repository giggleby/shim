...
class DrmVersion(Structure):
  ...
  def __repr__(self):
    return f"{self.name} {int(self.version_major)}.{int(self.version_minor)}.{int(self.version_patchlevel)} ({self.desc}) ({self.date})"
...
class DrmModeCrtc(Structure):
  ...
  def __repr__(self):
    return f"<CRTC ({int(self.crtc_id)})>"
...
def _screenshot(image, fb):
  ...
  if fb.depth == 24:
    ...
    raise RuntimeError(f"Couldn't unformat FB: {fb!r}")
