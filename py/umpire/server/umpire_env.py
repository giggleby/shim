...
class UmpireEnv:
  ...
  def AddPayload(self, file_path, payload_type: Union[str,
                                                      resource.PayloadTypes]):
    """Adds a cros_payload component into <base_dir>/resources.

    Args:
      file_path: file to be added.
      payload_type: An element of resource.PayloadTypes.
  ...
  def AddConfig(self, file_path, config_type: Union[str, resource.ConfigTypes]):
    """Adds a config file into <base_dir>/resources.

    Args:
      file_path: file to be added.
      config_type: An element of resource.ConfigTypes.

    Returns:
      Resource file name.
    """
    file_utils.CheckPath(file_path, 'source')
    res_name = resource.BuildConfigFileName(config_type, file_path)
    self._AddResource(file_path, res_name, False)
    return res_name

  def AddConfigFromBlob(self, blob, config_type: Union[str,
                                                       resource.ConfigTypes]):
    """Adds a config file into <base_dir>/resources.
    ...
    with file_utils.UnopenedTemporaryFile() as file_path:
      ...
      return self.AddConfig(file_path, config_type)
...
class UmpireEnvForTest(UmpireEnv):
  ...
  def __init__(self, port=None):
    ...
    self.AddConfigFromBlob('{}', resource.ConfigTypes.payload_config)
