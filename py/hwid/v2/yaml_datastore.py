...
class _DatastoreBase:
  ...
  def Decode(cls, data):
    ...
    try:
      ...
      raise InvalidDataError(f'YAML deserialization error: {e}')
  ...
  def ValidateSchema(cls, field_dict):
    ...
    def ValidateCollection(top_level_tag, collection_type_data,
      ...
      if len(collection_type_data) != 2:
        raise InvalidDataError(
            f'{cls.__name__!r} schema contains bad type definiton for element '
            f'{top_level_tag!r}, '
            'expected (collection type, field type) tuple, found '
            f'{repr(collection_type_data)}')
      collection_type, field_type = collection_type_data
      if collection_type not in [dict, list]:
        raise InvalidDataError(
            f'{cls.__name__!r} schema element {top_level_tag!r} has illegal '
            f'collection type {collection_type.__name__!r} '
            '(only "dict" and "list" are supported)')
      if not isinstance(collection_data, collection_type):
        raise InvalidDataError(
            f'{cls.__name__!r} schema validation failed for element '
            f'{top_level_tag!r}, '
            f'expected type {collection_type.__name__!r}, found '
            f'{type(collection_data).__name__!r}')
      if collection_type is dict:
        for field_key, field_data in collection_data.items():
          if not (isinstance(field_key, str) or isinstance(field_key, int)):
            raise InvalidDataError(
                f'{cls.__name__!r} schema validation failed for element '
                f'{top_level_tag!r}, '
                'dict key must be "str" or "int", found '
                f'{type(field_key).__name__!r}')
    ...
    def ValidateField(top_level_tag, field_type, field_data):
      ...
      raise InvalidDataError(
          f'{cls.__name__!r} schema validation failed for element '
          f'{top_level_tag!r}, expected type {field_type!r}, found '
          f'{type(field_data).__name__!r}')

    if set(cls._schema) ^ set(field_dict):
      raise InvalidDataError(
          f'{cls.__name__!r} schema and data dict keys do not match, '
          'data is missing keys: '
          f'{sorted(set(cls._schema) - set(field_dict))!r}, '
          f'data has extra keys: {sorted(set(field_dict) - set(cls._schema))!r}'
      )
