...
def RunIn(args, group_suffix=''):
  ...
  with TestGroup(id=group_id):
    ...
    # Requests tpm to be cleared in the next boot.
    # TPM should be cleared before finalize.
    # TODO(cychiang) clear tpm before finalize when chrome
    # is opened using telemetry.
    FactoryTest(
        id='RequestClearTPM',
        label_zh=u'请求清除 TPM',
        pytest_name='clear_tpm_owner_request')

    args.Barrier('RunInClearTPM',
                 pass_without_prompt=True,
                 accessibility=True)
    ...
    with AutomatedSequence(id='Stress', label_zh=u'集合压力测试'):
      ...
      FactoryTest(
          ...
          dargs=dict(
              ...
              ac_warning_countdown_secs=args.run_in_countdown_ac_secs,
              ac_type=args.run_in_countdown_ac_type))
    ...
    OperatorTest(
        ...
        dargs=dict(
           ...
           charger_type=args.run_in_countdown_ac_type,
           spec_list=[
