...
def FATP(args):
  ...
  with TestGroup(id='FATP'):
    ...
    # TODO(cychiang) Replace autotest with pytest version on master branch.
    OperatorTest(
        id='AudioJack',
        label_zh=u'音源孔',
    ...
    # TODO(cychiang) Remove the reboot step on master branchon master branch.
    with OperatorTest(id='ExtDisplay', label_zh=u'外接显示'):
      OperatorTest(
          id='ConnectDisplay',
          label_zh=u'连接显示器',
    ...
    if args.enable_fixture_tests:
      with TestGroup(id='Fixtures', label_zh=u'治具测试'):
        ...
        # TODO(henryhsu) replace this with pytest.
        OperatorTest(
            id='AudioQuality',
            label_zh=u'音源品質',
            autotest_name='factory_AudioQuality',
            run_if=args.SelectedForSampling('fatp_audio_fixture'),
            dargs=dict(
                # Mixer settings to enable digital mic
                dmic_switch_mixer_settings=[
                    {'name': '"DIGMICL Switch"',
                     'value': 'on'},
                    {'name': '"DIGMICR Switch"',
                     'value': 'on'},
                ...
                # Enable analog mic by default
                    ...
                    {'name': '"MIC2 Boost Volume"',
                     'value': '0'},
                    # Set playback gain to 0dB
                    {'name': '"Speaker Volume"',
                     'value': '21'},
                    {'name': '"Headphone Volume"',
                     'value': '26'}],
                # Mixer settings to enable speaker
                unmute_speaker_mixer_settings=[
                    {'name': '"Speaker Switch"',
                     'value': 'on,on'},
                    {'name': '"Headphone Switch"',
                     'value': 'off,off'},
                    ...
                    {'name': '"Right HP Mixer Left DAC1 Switch"',
                     'value': 'off'}],
                # Mixer settings to mute left/right channel
                mute_right_mixer_settings=[
                    {'name': '"Headphone Switch"',
                     'value': 'on,off'},
                    {'name': '"Left HP Mixer Left DAC1 Switch"',
                ...
                mute_left_mixer_settings=[
                    ...
                    {'name': '"Right HP Mixer Left DAC1 Switch"',
                     'value': 'on'}],
                loop_buffer_count=20))

        # Checks camera performance.
        # TODO(jchuang) replace this with pytest.
        OperatorTest(
