...
class BarcodeScanToFileTest(unittest.TestCase):
  ...
  def runTest(self):
    ...
    # Some existing test lists are written in UTF-8 str (instead
    # of u'string' form) and we didn't specify encoding in mai
    # program, so these labels will become ASCII encoding and
    # then fail when we want to compose an Unicode string later.
    if type(label_zh) is str:
      label_zh = label_zh.decode('utf-8')
