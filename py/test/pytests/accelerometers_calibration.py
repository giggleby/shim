...
ideal value of a sensor having 12bit analog-to-digital resolution and
...
class AccelerometerController(object):
  ...
  def __init__(self, iio_bus_id, spec_offset, spec_ideal_values, sample_rate):
    ...
    sysfs_init = [
        ...
        SYSFS_VALUE(add_trigger_path, self.trigger_number),
        # Link the trigger we created to our accelerometer device.
        SYSFS_VALUE(
            current_trigger_path, 'sysfstrig' + self.trigger_number)]
  ...
  def CleanUpCapture(self):
    ...
    self._SetSysfsValues(
        [SYSFS_VALUE(buffer_enable_path, '0'),
         SYSFS_VALUE(remove_trigger_path, self.trigger_number)],
        check_call=False)
