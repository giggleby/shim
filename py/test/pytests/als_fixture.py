...
- Please check als_fixture.json and als_fixture.schema.json
...
MSG_FIXTURE_CON = i18n_test_ui.MakeI18nLabelWithClass('Fixture Connected',
                                                      STYLE_PASS)
MSG_FIXTURE_DIS = i18n_test_ui.MakeI18nLabelWithClass('Fixture Disconnected',
                                                      STYLE_FAIL)
...
FAIL_ALS_CALCUL = 'AlsCalulcation'  # ALS coefficient calculation error.
...
class ALSFixture(unittest.TestCase):
    ...
    try:
      ...
      if self.args.control_chamber:
        if self.args.mock_mode:
          script = dict(
              [(k.strip(), v.strip())
               for k, v in reduce(lambda a, b: a + b,
  ...
  def _LogSerialNumber(self):
    testlog.AddArgument(state.KEY_ALL_SERIAL_NUMBERS,
                        state.GetAllSerialNumbers())

  def _LogArgument(self, key, value, description):
    testlog.AddArgument(key, value, description)
    self._Log("%s=%s" % (str(key), str(value)))
  ...
  def _ALSTest(self):
    ...
    try:
      ...
      self._LogFailure(FAIL_ALS_CALCUL, 'Error calculating calibration'
  ...
  def _OnU2SInsertion(self, dev_path):
    del dev_path  # unused
    cnt = 0
    while cnt <= self.chamber_n_retries:
      try:
        self._SetupFixture()
    ...
    raise als_light_chamber.ALSLightChamberError('Error connecting to ALS'
                                                 ' chamber')
  ...
  def _SetFixtureStatus(self, status):
    if status == FIXTURE_STATUS.CONNECTED:
      label = MSG_FIXTURE_CON
    elif status == FIXTURE_STATUS.DISCONNECTED:
      label = MSG_FIXTURE_DIS
    else:
  ...
  def _ValidateALS(self, light_name):
    ...
    if not result and not self.args.mock_mode:
      raise ValueError('Error validating calibrated als, got %d out of'
                       ' range (%d, %d)', sampled_vlux, lower_bound,
                       upper_bound)

  def _CheckALSOrdering(self):
    if self.args.mock_mode:
      return
    for i, li in enumerate(self.config['luxs']):
      for j in range(i):
        if ((li > self.config['luxs'][j] and
             self.all_sampled_lux[j] >= self.all_sampled_lux[i]) or
            (li < self.config['luxs'][j] and
  ...
  def _SaveCalibCoefToVPD(self):
    ...
    if self.config['force_light_init']:
      ...
      # The light-init script doesn't act as expeted, this is a workaround.
