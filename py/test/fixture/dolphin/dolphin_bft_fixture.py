...
_RE_GET_GPIO = lambda g: re.compile(r'(1|0)\*?\s' + f'{g}', re.MULTILINE)
...
class DolphinBFTFixture(bft_fixture.BFTFixture):
  ...
  def Init(self, **port_params):
    ...
    if 'dolphin_host' in port_params:
      try:
        remote = (f"http://{port_params['dolphin_host']}:"
                  f"{port_params['dolphin_port']}")
        ...
        raise bft_fixture.BFTFixtureException(f'Failed to Init(). Reason: {e}')
  ...
  def SetDeviceEngaged(self, device, engage):
    ...
    """
    action_str = f"{'engage' if engage else 'disengage'} device {device}"
  ...
  def SetIOExpanderToDefault(self, default_value, readback=False):
    ...
    if readback:
      ...
      if byte != int(default_value, 16):
        raise bft_fixture.BFTFixtureException(
            f'Readout mismatch {byte} (expect: {default_value})')
  ...
  def SetPDDataRole(self, mode):
    ...
    if mode not in ['UFP', 'DFP']:
      raise bft_fixture.BFTFixtureException(f'Unsupported mode {mode}')
    ...
    if not is_ready:
      raise bft_fixture.BFTFixtureException(
          f'Failed to wait PD state ready in '
          f'{DELAY_BEFORE_RETRY * self._POLL_PD_MAX_RETRIES:.1f} seconds')

    if self.GetPDStateWithRetries(3).get('datarole') == mode:
      return

    self._Send('pd 0 swap data', f'swap datarole to {mode}')
  ...
  def SetGPIOValue(self, gpio, value):
    ...
    """
    self._Send(f'gpioset {gpio} {int(value)}', f'set gpio {gpio}={int(value)}')
  ...
  def SetFakeDisconnection(self, disconnect_secs):
    ...
    self._Send(f'fakedisconnect 0 {int(disconnect_msecs)}',
               f'set fake disconnection {int(disconnect_secs)} secs')
  ...
  def SetOutputBufferChannel(self, channel):
    ...
    """
    self._Send(f'chan {int(channel)}', f'set output channel to {int(channel)}')
  ...
  def SetUSBHubChargeStatus(self, enable):
    ...
    self._I2CWrite('0x01',
                   f'0x{_ChangeBit(byte, USB_DN_PWREN_OFFSET, bit_value):2x}')
  ...
  def GetGPIOValue(self, gpio):
    ...
    """
    self._Recv(0, 'clean buffer')
    self._Send(f'gpioget {gpio}', f'get gpio {gpio} value')
    time.sleep(0.1)  # Wait for message output
    read_output = self._Recv(0, 'read output')
    logging.info('read_output = %r', read_output)
    ...
    raise bft_fixture.BFTFixtureException(f'Cannot get gpio {gpio} value')
  ...
  def _I2CWrite(self, reg_address, value):
    ...
    """
    logging.info('Write I2C = i2cxfer w 1 %s %s %s', self._i2c_address,
                 reg_address, value)
    self._Send(f'i2cxfer w 1 {self._i2c_address} {reg_address} {value}',
               f'I2C write {self._i2c_address} {reg_address} = {value}')
  ...
  def _I2CRead(self, reg_address):
    ...
    """
    self._Recv(0, 'clean buffer')
    self._Send(f'i2cxfer r 1 {self._i2c_address} {reg_address}',
               f'I2C read {self._i2c_address} {reg_address}')
    time.sleep(0.1)  # Wait for message output
    read_output = self._Recv(0, 'read output')
