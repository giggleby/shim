...
class WhaleBFTFixture(bft.BFTFixture):
  ...
  def Init(self, **params):
    ...
    try:
      ...
      self._nuc_mlbsn_path = params.get('nuc_mlbsn_path')
  ...
  def ScanBarcode(self):
    ...
    mlbsn = process_utils.SpawnOutput(
        ssh_command_base + [self._nuc_host, 'cat', self._nuc_mlbsn_path])
    if not mlbsn:
      raise bft.BFTFixtureException('Unable to read barcode from %s:%s' %
                                    (self._nuc_host, self._nuc_mlbsn_path))
    return mlbsn.strip()
