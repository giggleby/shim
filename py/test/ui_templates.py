...
class Option:
  ...
  def GenerateHTML(self):
    """Generate HTML tag."""
    attrs = f'value="{self._value}"'
    if self._selected:
      attrs += ' selected'
    return [f'<option {attrs}>', self._display, '</option>']
...
class SelectBox:
  ...
  def GenerateHTML(self):
    """Generate HTML tags."""
    html = [
        f'<select id="{self._element_id}" size={int(self._size)} style="'
        f'{self._style}">'
    ]
...
class Table:
  ...
  def GenerateHTML(self):
    """Generates HTML tags."""
    html = [f'<table id="{self._element_id}" style="{self._style}">']
